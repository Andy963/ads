...
async def relayout_all_workflows() -> str:
    """
    重新布局所有工作流。
    ...
    try:
        ...
        with get_db() as db:
            ...
            all_edges = GraphCRUD.get_all_edges()

            target_ids = {edge.target for edge in all_edges}
            root_nodes = [node for node in all_nodes if node.id not in target_ids]

            # 2. 为每个根节点（工作流）重新布局
            updated_count = 0

            for workflow_index, root in enumerate(sorted(root_nodes, key=lambda n: n.created_at or '')):
                # 计算这个工作流的基础 Y 坐标
                base_y = AutoLayout.START_Y + workflow_index * AutoLayout.WORKFLOW_Y_SPACING

                # 使用 BFS 遍历这个工作流的所有节点
                to_visit = [(root.id, 0)]  # (节点ID, 深度)
                visited = set()
                ...
                while to_visit:
                    ...
                    # 计算位置
                    new_x = AutoLayout.START_X + depth * AutoLayout.HORIZONTAL_SPACING
                    new_y = base_y

                    # 更新节点位置
                    GraphCRUD.update_node(current_id, {"position": {"x": new_x, "y": new_y}})
                    updated_count += 1

                    # 找到子节点
                    child_edges = [e for e in all_edges if e.source == current_id]
        ...
        result = {
            "success": True,
            "statistics": {
                "workflows": len(root_nodes),
                "nodes_updated": updated_count
            },
            "message": f"成功重新布局 {len(root_nodes)} 个工作流，更新了 {updated_count} 个节点"
        }
