              ...
              <div class="row">
                ...
                <div class="row-actions">
                  ...
                  <button v-if="isActionAllowed(t, 'rerun') && canRerunTask(t) && editingId !== t.id"
                    class="iconBtn primary" type="button" title="重新执行" aria-label="重新执行任务" :disabled="Boolean(editingId)"
                  ...
                  <button v-if="editingId === t.id" class="iconBtn" type="button" title="取消编辑" aria-label="取消编辑任务"
                    @click.stop="stopEdit()">
                    <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                      <path fill-rule="evenodd"
