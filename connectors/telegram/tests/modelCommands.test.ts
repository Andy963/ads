...
const models: ModelOption[] = [
  { id: "one", modelId: "gpt-5.6-sol", displayName: "Sol", provider: "openai", isEnabled: true, isDefault: true, configJson: { reasoningEfforts: ["high", "xhigh", "max"] } },
...
describe("Telegram model commands", () => {
  ...
  it("finds enabled models only and respects configured reasoning efforts", () => {
    ...
    assert.deepEqual(allowedReasoningEfforts(models[0]!), ["high", "xhigh", "max"]);
    assert.deepEqual(allowedReasoningEfforts({ ...models[0]!, configJson: { reasoningEfforts: [] } }), ["high"]);
    assert.deepEqual(allowedReasoningEfforts({ ...models[0]!, configJson: { reasoningEfforts: ["max"] } }), ["max"]);
    assert.deepEqual(allowedReasoningEfforts({ ...models[0]!, configJson: { reasoningEfforts: ["bogus", "high"] } }), ["high"]);
  });
