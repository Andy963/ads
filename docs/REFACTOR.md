...
- `server/scheduler/runtime.ts`：重构（scheduler runtime 的 workspace key / payload 解析统一走 workspace root 归一化，避免同一 workspace 的 root path 与 nested path 分裂成多个调度域）。
...
- `server/tasks/storeImpl/taskOps.ts`：重构（抽取 `normalizeTaskWritableFields()` / `applyTaskLifecycleFields()`，统一 `createTask()` / `updateTask()` 的写路径归一化、write-once 字段与归档时间派生语义，保持存储格式不变）。
...
- `tests/scheduler/schedulerRuntime.test.ts`：更新（新增 nested workspace path 回归覆盖，锁定 scheduler runtime 对 workspace root 的 canonical key 语义）。
- `tests/tasks/taskStore.test.ts`：更新（新增 `promptInjectedAt` write-once、生命周期时间戳稳定性，以及 completed+reviewRequired 的 `archivedAt` 回归覆盖）。
