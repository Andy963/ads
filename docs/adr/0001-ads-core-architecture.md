...
- 整理日期：2025-12-22
- 适用范围：本仓库 `ads` 工具本身（CLI / Web Console / Telegram Bot / MCP Server），而非某个具体的 ADS workspace

## 背景 / 要解决的问题

ADS 的目标是把“规格驱动开发”（需求 → 设计 → 实施/验证）的流程固化为可执行的工作流，并让同一套工作流可以通过多个入口（本地 CLI、Web Console、Telegram Bot、MCP）触达。同时需要：

- 将工作流与上下文持久化（跨进程/跨会话可恢复）。
- 支持多模型/多代理（Codex/Claude/Gemini）与工具调用闭环。
- 在“能跑起来”的同时，尽量减少对用户项目的侵入，并提供目录白名单、review 锁等安全护栏。
...
**动机（从代码组织推断）：**
- 单一语言覆盖 CLI、WebSocket/HTTP 服务、Telegram Bot、MCP Server 等多入口。
- 与上游 SDK（如 `@openai/codex-sdk`、`@modelcontextprotocol/sdk`、Telegram `grammy` 等）天然贴合。
...
- 入口脚本：`src/ads.ts`（加载 env/log 后动态 import CLI）
- 主逻辑：`src/cli/index.ts`（解析 `/ads.*` 指令、驱动 orchestrator、打印状态）
- 注意：CLI 内存在“未检测到 workspace 时自动初始化”的逻辑（`src/cli/index.ts` 的 `ensureWorkspace`）。

#### 4.2 Web Console（HTTP + WebSocket）
...
#### 4.4 MCP Server（供外部 IDE/Agent 调用）

- 入口：`src/mcp/server.ts`
- transport：stdio（`@modelcontextprotocol/sdk`）
- 暴露工具：`ads_status` / `ads_init` / `ads_new` / `ads_commit` / `ads_branch` / `ads_log` / `ads_checkout` / `ads_rules` / `ads_search` 等（见 `src/mcp/server.ts` 注册逻辑）
...
- Review Lock：在 review 进行期间限制指令执行；允许的“安全指令”集合见 `src/utils/reviewLock.ts`（并在 `src/cli/index.ts` 中检查）
...
- **统一的工作流内核，多入口复用**：CLI/Web/Telegram/MCP 共享同一套 workflow/service、graph/storage。
...
- **CLI 自动初始化可能带来“意外写入”**：`src/cli/index.ts` 在缺少 marker 时会触发 `initWorkspace`（对只想“看看”而不想改 workspace 的用户不友好）。
