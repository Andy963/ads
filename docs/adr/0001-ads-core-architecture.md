...
- 支持多模型/多代理（Codex/Claude/Gemini），并通过各自 CLI 的内置工具能力完成“读写文件/执行命令/搜索”等闭环。
...
- 框架：`grammy`
- 交互体验：处理过程中发送一条“状态消息”，通过 edit 更新（仅展示最近 3 条命令）；在最终回复发送后清理该状态消息，避免刷屏与残留噪音（`src/telegram/adapters/codex/statusUpdater.ts`、`src/telegram/adapters/codex.ts`）。
- 关键护栏：
...
- Adapter 统一走 CLI：`src/agents/adapters/*CliAdapter.ts`
  - 通过 spawn 运行 CLI binary（`codex`/`claude`/`gemini`），读取 JSONL stream。
