...
- ADS 已引入 “skill” 作为可热加载的扩展机制（`$ADS_STATE_DIR/.agent/skills/<name>/SKILL.md`），更适合承载“可选、外部依赖强、更新频繁”的能力。
...
2. 将 Tavily 能力下沉为 skill：
   - skill：`$ADS_STATE_DIR/.agent/skills/tavily-research/`
   - CLI 脚本：`$ADS_STATE_DIR/.agent/skills/tavily-research/scripts/tavily-cli.cjs`
...
- 负向/风险：
  - 如需联网能力，运行环境需确保 `$ADS_STATE_DIR/.agent/skills/` 同步并配置必要的凭证。
...
- Skill: `$ADS_STATE_DIR/.agent/skills/tavily-research/SKILL.md`
