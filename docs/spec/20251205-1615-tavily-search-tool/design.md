...
> 更新（2026-02-11）：ADS 已移除内置 Tavily runtime 集成（`src/tools/search/**`）。联网搜索与 URL 抓取改为通过 skill 脚本 `.agent/skills/tavily-research/scripts/tavily-cli.cjs` 提供；如保留 `/search` 命令则仅作为脚本的 thin wrapper。
