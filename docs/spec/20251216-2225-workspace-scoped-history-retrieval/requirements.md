...
> 更新（2026-02-12）：ADS 不再支持用户侧 CLI 入口，且 Web/Telegram 不提供用户侧 slash commands。本文档中关于 CLI 与 slash commands 的内容仅作为历史背景描述，当前支持入口为 Web Console + Telegram Bot。
...
- In Scope：
  ...
  - 支持用户显式触发一次 workspace 历史搜索（跨端一致），用于核对/补充上下文。
  - 为 workspace 历史搜索提供可扩展的高性能实现：优先使用 SQLite FTS5 虚表（新增 FTS5 表/索引与必要的同步机制）。
  - 通过 `.env` 提供可配置项（默认值：最近 100 轮、2000 字符上限）。
  - 不删除、不覆盖任何数据库文件（只读查询 + 追加写入既有历史逻辑）。
- Out of Scope：
  - 向量检索/embedding、RAG 框架、外部向量库。
  - 除用于 workspace 历史搜索的 FTS5 虚表/同步机制外，不做其他 SQLite schema 变更（尤其不修改 `history_entries` 字段）。
...
### Requirement 6: 显式触发的 workspace 历史搜索
- 概述：用户可显式触发一次 workspace 范围的历史搜索，快速定位“以前说过什么”。

**User Story:** 作为用户，我希望能用一个简单命令搜索历史对话，以便在讨论新需求时快速找回具体内容。

#### Acceptance Criteria
- [ ] WHEN 用户显式触发一次 workspace 历史搜索（提供 query） THEN 系统对 workspace 历史进行检索并返回匹配结果摘要。
...
- [ ] 显式的历史搜索不应隐式触发“新需求检索流程”（即：该搜索本身是用户手动检索，不等价于新需求触发器）。

#### Validation Notes
- 手动验证：在 Telegram 写入一条含关键词的历史，再显式触发一次历史搜索（用同一关键词）能命中并返回。
