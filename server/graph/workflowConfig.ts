...
function resolveConfigPath(): string {
  ...
  const fallbackPaths = [
    ...
    path.join(PROJECT_ROOT, "server", "graph", "config.yaml"),
    // When running compiled code from dist, hop back to repo server/
    path.join(PROJECT_ROOT, "..", "server", "graph", "config.yaml"),
  ...
  throw new Error(
    "无法找到工作流配置文件。请设置 ADS_CONFIG_PATH 指向 config.yaml，或确保构建产物包含 dist/server/graph/config.yaml，或仓库存在 server/graph/config.yaml",
  );
}
