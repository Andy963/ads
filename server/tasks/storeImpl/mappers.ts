...
export function toTask(row: Record<string, unknown>): Task {
  ...
  return {
    ...
    nextAttemptAt: toNullableNumber(row.next_attempt_at),
...
export function toTaskRun(row: Record<string, unknown>): TaskRun {
  return {
    ...
    cleanupStatus: normalizeTaskCleanupStatus(row.cleanup_status),
    cleanupError: normalizeNullableString(row.cleanup_error),
    cleanupAt: toNullableNumber(row.cleanup_at),
