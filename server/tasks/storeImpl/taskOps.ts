...
export function createTaskStoreTaskOps(deps: { db: DatabaseType; stmts: TaskStoreStatements; workspaceId: string }) {
  ...
  const createTaskRun = (input: CreateTaskRunInput, now = Date.now()): TaskRun => {
    ...
    const run: TaskRun = {
      ...
      applyStatus: normalizeTaskApplyStatus(input.applyStatus ?? "skipped"),
      cleanupStatus: normalizeTaskCleanupStatus(
        input.cleanupStatus ?? (normalizeTaskExecutionIsolation(input.executionIsolation) === "required" ? "pending" : "not_required"),
      ),
      cleanupError: normalizeNullableString(input.cleanupError),
      cleanupAt: normalizeNullableFiniteNumber(input.cleanupAt),
  ...
  const updateTaskRun = (id: string, updates: Partial<Omit<TaskRun, "id" | "taskId">>, now = Date.now()): TaskRun => {
    ...
    const merged: TaskRun = {
      ...
      worktreeDir: normalizeNullableString(updates.worktreeDir ?? existing.worktreeDir),
      branchName: normalizeNullableString(updates.branchName ?? existing.branchName),
      baseHead: normalizeNullableString(updates.baseHead ?? existing.baseHead),
      endHead: normalizeNullableString(updates.endHead ?? existing.endHead),
      ...
      cleanupStatus: normalizeTaskCleanupStatus(updates.cleanupStatus ?? existing.cleanupStatus),
      cleanupError: updates.cleanupError === undefined ? existing.cleanupError : normalizeNullableString(updates.cleanupError),
      cleanupAt: updates.cleanupAt === undefined ? existing.cleanupAt : normalizeNullableFiniteNumber(updates.cleanupAt),
