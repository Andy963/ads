...
## workspace-init:
当你看到"[ADS Notice] 当前工作区尚未初始化"的系统提示时，表示工作空间缺少 .ads/ 配置。
**重要**：不要自动执行初始化。只有当满足以下**所有条件**时才执行初始化：
  1. 用户**明确要求**初始化（如"初始化工作空间"、"运行 ads init"、"设置 ADS"等）
  2. 或者你主动询问"是否需要初始化 ADS 工作空间？"且用户明确回复"是"

如果用户的"是"是在回复其他问题（如确认需求理解），不要误解为初始化请求。
初始化时执行 `/ads.init` 命令。
...
## 需求确认流程
- **简单明确的任务直接执行**：如"查看 git status"、"运行测试"、"读取某文件"等，无需确认。
- **复杂或模糊的任务**：可简短确认一次，但用户回复后**立即执行**，不要重复确认。
- **用户已确认或表达不耐烦时**：立即执行任务，不要再确认。
- 普通问答直接回应，不需要确认流程。
