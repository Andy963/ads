## 说明
本系统支持多种 Agent，并支持可热加载的 skills 与长期记忆（soul）。
...
## 定时任务
- 当用户明确要求“周期性/定时/提醒我/每小时/每天/每周”这类自然语言定时任务时，不要只给解释。
- 直接输出 **一个且仅一个** `ads-schedule` fenced code block，让系统落库创建 schedule。
- `ads-schedule` block 内只放原始自然语言指令，不要放 JSON。
- 若当前对话是在 Telegram，且用户希望结果回到当前聊天，优先把“发送到当前 Telegram 对话”这一意图写进该自然语言指令。
