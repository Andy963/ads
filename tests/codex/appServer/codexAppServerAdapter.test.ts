...
describe("CodexAppServerAdapter", () => {
  ...
  it("ignores raw reasoning textDelta events and emits zero live steps", async () => {
    const fake = buildFakeServer({
      autoReplies: {
        "thread/start": () => ({ thread: { id: "thread-v2-raw-text-delta" } }),
        "turn/start": () => ({}),
      },
    });
    const registry = new CodexAppServerDaemonRegistry({ factory: () => fake.client });
    const adapter = new CodexAppServerAdapter({ projectId: "v2-raw-text-delta", registry });
    ...
    const sendPromise = adapter.send("exercise raw reasoning text");
    await waitForRequestCount(fake, "turn/start", 1);
    fake.notify("turn/started", { threadId: "thread-v2-raw-text-delta", turn: { id: "turn-v2-raw" } });
    // Raw chain-of-thought tokens stay internal to model execution: the
    // adapter must not buffer, parse, or rebroadcast them as live steps.
    fake.notify("item/reasoning/textDelta", {
      threadId: "thread-v2-raw-text-delta",
      turnId: "turn-v2-raw",
      itemId: "reasoning-raw",
      delta: "Let me inspect the failing test first. ",
    });
    fake.notify("item/reasoning/textDelta", {
      threadId: "thread-v2-raw-text-delta",
      turnId: "turn-v2-raw",
      itemId: "reasoning-raw",
      delta: "Then I will run the checks.",
    });
    fake.notify("item/completed", {
      item: {
        type: "reasoning",
        id: "reasoning-raw",
        text: "Let me inspect the failing test first. Then I will run the checks.",
      },
      threadId: "thread-v2-raw-text-delta",
      turnId: "turn-v2-raw",
    });
    fake.notify("item/completed", {
      item: { type: "agentMessage", id: "msg-raw", text: "Done." },
      threadId: "thread-v2-raw-text-delta",
      turnId: "turn-v2-raw",
    });
    fake.notify("turn/completed", { threadId: "thread-v2-raw-text-delta", turn: { id: "turn-v2-raw" } });
    await sendPromise;

    assert.equal(events.some((event) => event.liveStep), false);
    assert.equal(
      events.some((event) => (event.delta ?? "").includes("inspect the failing test")),
      false,
    );
