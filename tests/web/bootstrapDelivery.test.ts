...
describe("web/ws/bootstrapDelivery", () => {
  ...
  it("skips reviewer history replay when no valid snapshot binding is available", () => {
    const sent: unknown[] = [];
    const historyStore = new HistoryStore({ namespace: "test-bootstrap-delivery-missing-binding", maxEntriesPerSession: 20 });
    historyStore.add("history-1", { role: "user", text: "stale reviewer context", ts: 1 });
