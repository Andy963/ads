...
describe("web/promptQueueService", () => {
  ...
  it("reports unconsumed rows in a lane snapshot", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ads-prompt-queue-snapshot-"));
    const db = new DatabaseConstructor(path.join(tempDir, "state.db"));
    const store = createPromptQueueStore(db);
    const lane = {
      authUserId: "auth-1",
    ...
    const completed = store.enqueue({ ...lane, clientMessageId: "done", payload: { text: "done" } });
    const running = store.enqueue({ ...lane, clientMessageId: "running", payload: { text: "running" } });
    const failed = store.enqueue({ ...lane, clientMessageId: "failed", payload: { text: "failed" } });
    store.enqueue({ ...lane, clientMessageId: "still-queued", payload: { text: "queued" } });
    db.prepare("UPDATE prompt_queue SET status='completed', completed_at=?, updated_at=? WHERE id=?").run(Date.now(), Date.now(), completed.entry.id);
    db.prepare("UPDATE prompt_queue SET status='running', started_at=?, updated_at=? WHERE id=?").run(Date.now(), Date.now(), running.entry.id);
    db.prepare("UPDATE prompt_queue SET status='failed', completed_at=?, updated_at=?, last_error=? WHERE id=?").run(Date.now(), Date.now(), "failed", failed.entry.id);
    ...
    assert.deepEqual(
      service.getSnapshot(lane).map((entry) => entry.clientMessageId),
      ["running", "failed", "still-queued"],
    );
    db.close();
    fs.rmSync(tempDir, { recursive: true, force: true });
  });
