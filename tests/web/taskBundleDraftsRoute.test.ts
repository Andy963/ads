...
describe("web/api/task-bundle-drafts", () => {
  ...
  it("does not expose draft spec routes", async () => {
    const auth = { userId: "u-1", username: "u" };
    const req = createReq("GET");
    const res = createRes();
    const pathname = "/api/task-bundle-drafts/d-1/spec";
    const url = new URL(`http://localhost${pathname}?workspace=/tmp/ws`);

    assert.equal(
      await handleTaskBundleDraftRoutes(
        { req: req as any, res: res as any, url, pathname, auth } as any,
        {} as any,
      ),
      false,
    );
