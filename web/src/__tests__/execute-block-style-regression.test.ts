...
describe("execute block style regression", () => {
  it("keeps a fixed execute block height and consistent stack spacing", () => {
    ...
    expect(sfc).toMatch(/\.execute-underlay\[data-layer="1"\]\s*\{[\s\S]*?transform:\s*translate\(0,\s*6px\)\s*;[\s\S]*?\}/);
    expect(sfc).toMatch(/\.execute-underlay\[data-layer="2"\]\s*\{[\s\S]*?transform:\s*translate\(0,\s*12px\)\s*;[\s\S]*?\}/);

    expect(sfc).toMatch(/\.execute-header\s*\{[\s\S]*?flex-wrap:\s*nowrap\s*;[\s\S]*?\}/);
    expect(sfc).toMatch(/\.execute-cmd\s*\{[\s\S]*?text-overflow:\s*ellipsis\s*;[\s\S]*?\}/);
    expect(sfc).toMatch(/\.execute-output\s*\{[\s\S]*?overflow:\s*auto\s*;[\s\S]*?\}/);
  });
});
