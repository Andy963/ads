...
describe("TaskDetail Accessibility", () => {
  ...
  it("renders aria-labels on action buttons", () => {
    ...
    expect(refreshBtn.attributes("aria-label")).toBe("Refresh task");

    const cancelBtn = wrapper.find('button[title="终止"]');
    expect(cancelBtn.attributes("aria-label")).toBe("Cancel task");

    const retryBtn = wrapper.find('button[title="重试"]');
    expect(retryBtn.attributes("aria-label")).toBe("Retry task");

    const deleteBtn = wrapper.find('button[title="删除任务"]');
    expect(deleteBtn.attributes("aria-label")).toBe("Delete task");
  });
  ...
  it("renders aria-label on status container", () => {
    ...
    expect(status.attributes("aria-label")).toBe("Status: pending");
  ...
  it("renders aria-labels on composer inputs", () => {
    ...
    expect(textarea.attributes("aria-label")).toBe("New instruction");

    const sendBtn = wrapper.find("button.send");
    expect(sendBtn.attributes("aria-label")).toBe("Send instruction");
  });
});
