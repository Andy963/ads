  ...
  <div v-else class="detail">
    <div class="header">
      <div class="header-left">
        ...
        <div class="meta">
          <span class="status" :data-status="task.status" :title="task.status" :aria-label="`Status: ${task.status}`" role="img">
      ...
      <div class="actions">
        <button class="iconBtn" type="button" title="刷新" aria-label="Refresh task" @click="emit('refresh', task.id)">
        ...
        <button class="iconBtn danger" type="button" title="终止" aria-label="Cancel task" :disabled="!canCancel" @click="emit('cancel', task.id)">
          <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        ...
        <button class="iconBtn primary" type="button" title="重试" aria-label="Retry task" :disabled="!canRetry" @click="emit('retry', task.id)">
          <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        ...
        <button class="iconBtn danger" type="button" title="删除任务" aria-label="Delete task" :disabled="task.status === 'running' || task.status === 'planning'" @click="emit('delete', task.id)">
          <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    <div class="composer">
      ...
      />
      <button class="send" :disabled="!input.trim()" type="button" aria-label="Send instruction" @click="send">Send</button>
    </div>
  </div>
</template>
